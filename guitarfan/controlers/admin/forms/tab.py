...
from flask.ext.wtf import Form, StringField, TextField, HiddenField, BooleanField, PasswordField, SubmitField, \
    SelectField, FileField, QuerySelectField, QuerySelectMultipleField, HiddenInput, Required, FileAllowed, FileRequired, EqualTo, Regexp, Email, length
...
class TabFrom(Form):
    ...
    title = TextField(u'Title', validators=[Required(message=u'Name is required')])
    ...
    style = SelectField(u'Music Style', choices=TabFormat.get_described_items(), default=1, coerce=int)
    ...
    audio_url = TextField(u'Audio Url',
                          validators=[Regexp(u'^(http|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?',
                          message=u'Url is not correct')])
